...
*   - expectedDevices  | Integer | Optional | Default to MAX_INT
* ---------------------------------------------------------------- */
LifxLanUdp.prototype.discover = function (params) {
	let promise = new Promise((resolve, reject) => {
		...
		let expectedDevices = param_check_result['params']['expectedDevices'];
		...
		this._netif_list.forEach((netif) => {
			...
			this._requests[seq] = (res) => {
				...
				if (!devices[addr]) {
                    deviceCount++;
					devices[addr] = res;
					// if the number of expected devices is reached, return immediately
		...
		// Timer
		let timer = setTimeout(responseHandler, wait);
...
LifxLanUdp.prototype._checkDiscoverParams = function (params) {
	...
	let wait = 3000;
	let expectedDevices = Number.MAX_SAFE_INTEGER;
	if ('wait' in params) {
	...
	if ('expectedDevices' in params) {
        if (typeof (params['expectedDevices']) === 'number' && params['expectedDevices'] > 0 || params['expectedDevices'] % 1 === 0) {
            expectedDevices = params['expectedDevices'];
        } else {
            return { error: new Error('The parameter `expectedDevices` is invalid.') };
        }
    }
	let p = {
		wait: wait,
